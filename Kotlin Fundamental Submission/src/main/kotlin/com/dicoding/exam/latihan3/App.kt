...
fun <T> checkType(args: T): String {
    var tipeData = "Tipe Data"
    when (args) {
        is Int -> { tipeData = "Integer" }
        is String -> { tipeData = "String" }
        is Boolean -> { tipeData = "Boolean" }
        is Double -> { tipeData = "Double" }
        is List<*> -> { tipeData = "List" }
        is Map<*, *> -> { tipeData = "Map" }
    }
    return "Yes! it's $tipeData"
}
